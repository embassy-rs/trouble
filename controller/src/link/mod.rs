...
pub struct LinkLayer<R> where R: Radio {
    state: State,
    address: Address,
    radio: C::Radio,
}
...
impl<R> LinkLayer<R> where R: Radio {
    pub fn new(address: Address, radio: Radio) -> Self {
        Self {
            state: State::Standby,
            radio,
            address,
    ...
    pub async fn advertise(&mut self, data: &[u8], config: &AdvertisingConfig) -> impl Future<Output = Result<(), Error>> {
    ...
    pub fn scan(&mut self, data: &mut [u8], config: &ScanConfig) -> impl Future<Output = Result<usize, Error>> {
