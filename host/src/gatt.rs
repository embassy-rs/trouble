...
impl<'stack, 'server, P: PacketPool> GattEvent<'stack, 'server, P> {
    pub(crate) fn new(data: GattData<'stack, P>, server: &'server dyn DynamicAttributeServer<P>) -> Self {
...
where
    P: PacketPool,
{
    // Need to get it before we take the PDU
    let handle = data.handle().unwrap_or(0);

    if let Some(pdu) = data.pdu.take() {
        let res = match result {
            Ok(_) => process_accept(&pdu, &data.connection, server),
            // As per spec, if the incoming ATT does not have an ATT handle, we should report with handle 0
            Err(code) => process_reject(&pdu, handle, &data.connection, code),
        };
        res
    } else {
