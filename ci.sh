...
RUSTFLAGS="--cfg getrandom_backend=\"custom\"" \
  cargo build --release --manifest-path examples/esp32/Cargo.toml --target riscv32imc-unknown-none-elf --features esp32c3,security \
    --bin ble_bas_peripheral_sec \
    --bin ble_bas_central_sec \
    --bin ble_bas_peripheral_bonding

RUSTFLAGS="--cfg getrandom_backend=\"custom\"" \
  cargo build --release --manifest-path tester/nrf52/Cargo.toml --target thumbv7em-none-eabihf --features nrf52840
