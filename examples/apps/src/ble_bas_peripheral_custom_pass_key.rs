    ...
    C: Controller,
{
    ...
    // Set a fixed passkey (e.g., 123456) for PassKey Entry pairing.
    // This passkey will be displayed during pairing instead of a random one.
    // Set to None to use random passkey generation (default behavior).
    let fixed_passkey: Option<u32> = Some(123456);

    let mut resources: HostResources<DefaultPacketPool, CONNECTIONS_MAX, L2CAP_CHANNELS_MAX> = HostResources::new();
    let stack = trouble_host::new(controller, &mut resources)
        .set_random_address(address)
        .set_io_capabilities(IoCapabilities::DisplayOnly)
        .set_passkey(fixed_passkey)
        .build();
    ...
    if fixed_passkey.is_some() {
        info!("Using fixed passkey: {:?}", fixed_passkey);
    }
...
async fn gatt_events_task(server: &Server<'_>, conn: &GattConnection<'_, '_, DefaultPacketPool>) -> Result<(), Error> {
    ...
    let reason = loop {
        match conn.next().await {
            ...
            GattConnectionEvent::PassKeyDisplay(pk) => {
                info!("[gatt] PassKey displayed to user: {}", pk.value());
                info!("[gatt] Please enter this passkey on the central device");
            }
